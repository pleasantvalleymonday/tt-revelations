...
parser.add_argument('--compile-cxx', '-c', action='store_true',
                    help='Compile the CXX codes and generate journey.exe into built.')
parser.add_argument('--make-nri', '-n', action='store_true',
                    help='Generate journey NRI.')
...
class JourneyPackager(NiraiPackager):
    HEADER = 'TTJOURNEY'
    BASEDIR = '..' + os.sep
    ...
    def generate_niraidata(self):
        ...
        # DC
        niraidata += '\nDC = %r' % self.get_file_contents('../dependencies/astron/dclass/journey.dc', True)
...
# Compile the engine
if args.compile_cxx:
    compiler = NiraiCompiler('journey.exe', libs=set(glob.glob('libpandadna/libpandadna.dir/Release/*.obj')))

    compiler.add_nirai_files()
    compiler.add_source('src/journey.cxx')

    compiler.run()

# Compile the game data
if args.make_nri:
    pkg = JourneyPackager('built/TTJData.bin')
