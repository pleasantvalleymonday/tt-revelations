...
class SiteUploadFSM(FSM):
    ...
    URL = config.GetString('toptoons-api-endpoint', 'http://toontownjourney.com/toptoons/post/')
...
class TopToonsManagerUD(DirectObject):
    ...
    def __init__(self, air):
        ...
        coll = None
        if self.air.dbConn:
            coll = self.air.dbGlobalCursor.journeyToons.find_one({'month': self.__curMonth})
            if not coll:
                lastMonthColl = self.air.dbGlobalCursor.journeyToons.find_one({'month': getPrevMonth()})
                if lastMonthColl:
    ...
    def saveSite(self):
        if self.air.dbConn:        
            self.air.dbGlobalCursor.journeyToons.update({'month': self.__curMonth}, {'$set': self.__topToonsData}, upsert=True)
